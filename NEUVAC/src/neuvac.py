...
def neuvacEUV(f107, f107a, bandLim=False, tableFile=None, statsFiles=None):
    """
    ...
    :param tableFile: str
        ...
        not given, simply uses the old coefficients in this module.
    ...
    :return perturbedEuvFlux: ndarray
        A nxm ndarray where n is the number of EUV flux values perturbed due to inherent uncertainty and m is the
    ...
    if tableFile is None:
        neuvacTable = np.flipud(waveTable)
    else:
        neuvacTable = []
    ...
    RidleySlopes = neuvacTable[:, 2:5]
    RidleyPowers = neuvacTable[:, 6:]
    RidleyIntercepts = neuvacTable[:, 5]
    WAVES = neuvacTable[:, 0]
    WAVEL = neuvacTable[:, 1]
    # Include statistical data for calculating uncertainties:
    ...
    if bandLim:  # Returns values ONLY for those corresponding to the wavelengths used by EUVAC
        return euvFlux[:, 7:44], euvIrradiance[:, 7:44], perturbedEuvFlux[:, 7:44], perturbedEuvIrradiance[:, 7:44]
    else:
        return euvFlux, euvIrradiance, perturbedEuvFlux, perturbedEuvIrradiance
